	...
	protected void doCommit(DefaultTransactionStatus status) {
		...
		try {
			...
			if(Hazelcast.getAllHazelcastInstances().size()==0){
				...
				sqlist = (List) Hazelcast.getHazelcastInstanceByName("hazelcast-instance").getMap("TxCache").get("syn_sqls"+tread_id);
				if(sqlist!=null){
					//删除缓存
					//Map<String, List> mapp= Hazelcast.getHazelcastInstanceByName("hazelcast-instance").getMap("TxCache");
					Hazelcast.getHazelcastInstanceByName("hazelcast-instance").getMap("TxCache").remove("syn_sqls"+tread_id);
					//事务完成之后，加入Hazelcast的生产者队列。
					try {
						Hazelcast.getHazelcastInstanceByName("hazelcast-instance").getQueue("CommitedSqlist").put(sqlist);
